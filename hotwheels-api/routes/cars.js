...
// Rota para o endpoint GET que retorna um carro pelo ID
router.get('/:id', (req, res) => {
  ...
  fs.readFile(filePath, 'utf8', (err, data) => {
    ...
    if (!car) {
      return res.status(404).json({ message: 'Erro ao analisar arquivo JSON' });
    }
    res.json(car);
  });
});
...
// Rota para o endpoint PUT que atualiza um carro pelo ID
router.put('/:id', (req, res) => {
  ...
  fs.readFile(filePath, 'utf8', (err, data) => {
    ...
    // Atualiza o carro com os novos dados
    carsData[carIndex] = { ...carsData[carIndex], ...updatedCar };

    // Escreve os dados atualizados no arquivo JSON
    fs.writeFile(filePath, JSON.stringify(carsData), err => {
      if (err) {
        return res.status(500).json({ message: 'Erro ao analisar arquivo JSON' });
      }
